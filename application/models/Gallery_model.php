...
class Gallery_model extends CI_Model {
    ...
    function __construct($options = null) {
        parent::__construct();
        $this->response = array();
        $this->package_id = $this->uri->segment(4);
        $this->options = array(
            'script_url' => site_url('admin/packages/upload_gallery/' . $this->package_id),
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/uploads/gallery/' . $this->package_id . '/',
            'upload_url' => $this->get_full_url() . '/uploads/gallery/' . $this->package_id . '/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    function insertData($file_name) {
        if (!empty($file_name) && $this->package_id != "") {
            $data = array('image' => $file_name, 'package_id' => $this->package_id, 'is_active' => '1');
            if ($this->db->insert('packages_images', $data)) {
                return true;
            }
        }
        return false;
    ...
    function deleteData($file_name) {
        if (!empty($file_name)) {
            ...
            if ($this->db->delete('packages_images', $data)) {
                return true;
            }
        }
        return false;
